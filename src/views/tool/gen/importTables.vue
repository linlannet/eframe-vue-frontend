<template>
	<div>
		<a-modal v-model:visible="pageDataObj.visible" width="60%" :title="pageDataObj.title" :destroyOnClose="true"
...
<script setup>
	...
	const queryTableData = () => {
		...
		LocalUtil.getTableDataByQueryFuncV2(listDbTable(queryFormObj.data), tableObj);
	}
	...
	const clickModalOk = (e) => {
		...
		if (tableNames == '') {
			LocalUtil.messageWarn('请选择要导入的表！');
			return;
		}
		importTable({
			tables: tableNames
		}).then((res) => {
			LocalUtil.messageSuccess('导入表操作成功。');
			if (res.code === 200) {
